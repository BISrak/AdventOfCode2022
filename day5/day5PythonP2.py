...
for line in input:
    ...
    if (current_line > 9):
        ...
        for i in range (int(instructions[1])):
            temp_container.append(list_of_stacks[int(instructions[3]) - 1].pop())
            
        temp_container.reverse()
        list_of_stacks[int(instructions[5]) - 1] = list_of_stacks[int(instructions[5]) - 1] + temp_container 
    current_line += 1
